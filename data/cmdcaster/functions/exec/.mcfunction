...
# 通知
    execute if data storage indexstorage: Config.CmdCaaster{NotifyResult: 1b} run tellraw @s [{"translate": "Ashen.CmdCaster.Result", "fallback": "Executed %s command(s) from Command Caster","with": [{"score": {"name": "$Successes","objective": "Ashen.CmdCaster.Temporary"}}]}]
